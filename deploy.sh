...
# Set query tag for tracking (applies to this session)
QUERY_TAG='{"origin":"sf_sit-is","name":"sfguide_ai_powered_predictive_grid_maintenance","version":{"major":1,"minor":0},"attributes":{"is_quickstart":1,"source":"sql"}}'
...
execute_sql() {
    ...
    if [ "$SQL_CMD" = "snow sql" ]; then
        {
            echo "-- Set query tag for tracking"
            echo "ALTER SESSION SET query_tag = '${QUERY_TAG}';"
            echo ""
            cat "$file"
        } | snow sql -c "$CONNECTION" -i --enable-templating NONE
    else
...
# Phase 5: Intelligence Agents (MOVED TO END OF PHASE 7)
# Agent creation now happens after Cortex Search Services are created (Phase 7)
# This prevents "Cortex Search Service does not exist" errors when agent tries to connect
echo -e "${BLUE}═══ Phase 5: Intelligence Agents ═══${NC}"
echo -e "${YELLOW}  ℹ Agent deployment deferred to Phase 7 (after search services created)${NC}"
echo ""
...
# Deploy Intelligence Agent (AFTER search services exist)
if [ "$SKIP_AGENTS" = false ]; then
    echo -e "${YELLOW}▶ Deploying Intelligence Agent (after search services created)...${NC}"
    ...
    if [ "$SQL_CMD" = "snow sql" ]; then
        ...
        if [ -n "$AGENT_CHECK" ]; then
            echo -e "${GREEN}  ✓ Intelligence Agent created successfully${NC}"
            
            # Register agent with Snowflake Intelligence UI
            echo -e "${YELLOW}  → Registering agent with Intelligence UI...${NC}"
            snow sql -c "$CONNECTION" -q "ALTER SNOWFLAKE INTELLIGENCE IDENTIFIER('SNOWFLAKE_INTELLIGENCE_OBJECT_DEFAULT') ADD AGENT IDENTIFIER('${DATABASE}.ANALYTICS.\"Grid Reliability Intelligence Agent\"');" --enable-templating NONE > /dev/null 2>&1 || true
            
            # Grant Intelligence object usage to roles
            snow sql -c "$CONNECTION" -q "GRANT USAGE ON SNOWFLAKE INTELLIGENCE SNOWFLAKE_INTELLIGENCE_OBJECT_DEFAULT TO ROLE GRID_ANALYST;" --enable-templating NONE > /dev/null 2>&1 || true
            snow sql -c "$CONNECTION" -q "GRANT USAGE ON SNOWFLAKE INTELLIGENCE SNOWFLAKE_INTELLIGENCE_OBJECT_DEFAULT TO ROLE GRID_OPERATOR;" --enable-templating NONE > /dev/null 2>&1 || true
            snow sql -c "$CONNECTION" -q "GRANT USAGE ON SNOWFLAKE INTELLIGENCE SNOWFLAKE_INTELLIGENCE_OBJECT_DEFAULT TO ROLE GRID_ML_ENGINEER;" --enable-templating NONE > /dev/null 2>&1 || true
            
            echo -e "${GREEN}  ✓ Agent registered in Intelligence UI (Projects → Intelligence)${NC}"
        else
